...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for page in orm['cms.Page'].objects.all():
            if page.application_urls == 'MailChimpApp':
                page.application_urls = None
                page.save()

        # rename plugins
        for plugin in orm['cms.cmsplugin'].objects.all():
            if plugin.plugin_type == 'SubscriptionPlugin':
                plugin.plugin_type = 'SubscriptionCMSPlugin'
                plugin.save()
    ...
    complete_apps = ['cms', 'aldryn_mailchimp']
    symmetrical = True
